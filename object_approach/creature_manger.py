...
class CreatureManager:
    ...
    def get_Creature_byName(self, creature_name : str):
        with self._connect() as conn:
            if not conn:
                print("Could not connect to database.")
                return
        ...
        for i in total_records:  
            tuple_db_record = cursor.fetchall()
            obj_creature.set_creature_id(tuple_db_record[0])
            obj_creature.set_name(tuple_db_record[1])
            obj_creature.set_category(tuple_db_record[2])
            obj_creature.set_biomes(tuple_db_record[3].split(',') if tuple_db_record[3] else [])
            obj_creature.set_behavior(tuple_db_record[4])
            obj_creature.set_danger_level(tuple_db_record[5])
            obj_creature.set_depth_level(tuple_db_record[6])
            obj_creature.set_pda_entry(tuple_db_record[7])
            obj_creature.set_img_url(tuple_db_record[8])
