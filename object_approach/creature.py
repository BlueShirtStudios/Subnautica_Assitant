class Creature:
    ...
    def displayCreatureInfo(self):
        print(f"Name: {self.name}")
        print(f"Category: {self.category}")
        print(f"Biomes: {self.biomes}")
        print(f"Behavior: {self.behavior}")
        print(f"Danger_Level: {self.depth_level}")
        print(f"Depth-Level: {self.depth_level}")
        print(f"PDA Entry: {self.pda_entry}")
        print(f"Image URL: {self.img_url}")
    
    @classmethod
    def row_from_DB(cls, row_tuple : tuple):
        #Make sure that tuple is not empty, if empty it will return None
        if not row_tuple:
            return None   
        
        #Convert single string in string list for class
        biomes_list = row_tuple[3].split(',') if row_tuple[3] else []
        
        #Assign the row details to the class
        return cls(
            creature_id = row_tuple[0],
            name = row_tuple[1],
            category = row_tuple[2],
            biomes = biomes_list,    
            behavior = row_tuple[4],
            danger_level = row_tuple[5],
            depth_range = row_tuple[6],
            pda_entry = row_tuple[7],
            image_url = row_tuple[8]
        )
